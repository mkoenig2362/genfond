from typing import Optional, TextIO

import mergedeep
import yaml

DEFAULT_CONFIG = {
    "problem_type": "QNP", #alternative: "FOND"
    "min_complexity": 2,
    "max_complexity": 35,
    "policy_iterations": 100,
    "abort_on_cycle": False,
    "stop_after_first_solution": True,
    "policy_steps": 10000,
    "num_threads": None,
    "max_memory": None,
    "dump_failed_policies": False,
    "dump_clingo_program": None,
    "keep_going": False,
    "continue_after_error": False,
    "include_boolean_features": True,
    "include_numerical_features": True,
    "include_concepts": False,
    "include_roles": False,
    "include_actions": False,
    "include_action_params": False,
    "include_pristine_states": True,
    "include_dead_states": True,
    "include_goal_states": True,
    "preset_features": None,
    "generate_space_by_state" : False,
    "prune_roles": True,
    "prune_concepts": True,
    "prune_features": True,
    "use_selected_states": False,
    "use_random_walks": False,
    "use_unrestricted_features": True,
    "unselect_problems": False,
    "feature_generator": {
        "generate_empty_boolean": True,
        "generate_inclusion_boolean": False,
        "generate_nullary_boolean": True,
        "generate_all_concept": False,
        "generate_and_concept": False,
        "generate_bot_concept": False,
        "generate_diff_concept": False,
        "generate_equal_concept": False,
        "generate_not_concept": True,
        "generate_one_of_concept": False,
        "generate_or_concept": False,
        "generate_primitive_concept": True,
        "generate_projection_concept": False,
        "generate_some_concept": False,
        "generate_subset_concept": False,
        "generate_top_concept": False,
        "generate_concept_distance_numerical": False,
        "generate_count_numerical": True,
        "generate_and_role": False,
        "generate_compose_role": False,
        "generate_diff_role": False,
        "generate_identity_role": False,
        "generate_inverse_role": False,
        "generate_not_role": False,
        "generate_or_role": False,
        "generate_primitive_role": False,
        "generate_restrict_role": False,
        "generate_til_c_role": False,
        "generate_top_role": False,
        "generate_transitive_closure_role": False,
        "generate_transitive_reflexive_closure_role": False,
        "generate_primitive_frame_unary": True,
        "generate_primitive_frame_binary": False,
        "generate_restrict_frame_unary": True,
        "generate_restrict_frame_binary": False,
        "generate_distance_frame": True,
        "generate_minimum_numerical": True,
        "generate_maximum_numerical": True,
        "generate_sum_frame_numerical": True,
    },
    "unrestricted_feature_generator": {
        "generate_empty_boolean": True,
        "generate_inclusion_boolean": False,
        "generate_nullary_boolean": True,
        "generate_all_concept": False,
        "generate_and_concept": False,
        "generate_bot_concept": False,
        "generate_diff_concept": False,
        "generate_equal_concept": False,
        "generate_not_concept": True,
        "generate_one_of_concept": False,
        "generate_or_concept": False,
        "generate_primitive_concept": True,
        "generate_projection_concept": False,
        "generate_some_concept": False,
        "generate_subset_concept": False,
        "generate_top_concept": False,
        "generate_concept_distance_numerical": False,
        "generate_count_numerical": True,
        "generate_and_role": False,
        "generate_compose_role": False,
        "generate_diff_role": False,
        "generate_identity_role": False,
        "generate_inverse_role": False,
        "generate_not_role": False,
        "generate_or_role": False,
        "generate_primitive_role": False,
        "generate_restrict_role": False,
        "generate_til_c_role": False,
        "generate_top_role": False,
        "generate_transitive_closure_role": False,
        "generate_transitive_reflexive_closure_role": False,
        "generate_primitive_frame_unary": True,
        "generate_primitive_frame_binary": False,
        "generate_restrict_frame_unary": True,
        "generate_restrict_frame_binary": False,
        "generate_distance_frame": True,
        "generate_minimum_numerical": True,
        "generate_maximum_numerical": True,
        "generate_sum_frame_numerical": True,
    },
    "log": {
        "execution": "CRITICAL",
    },
}

DEFAULT_TYPE_CONFIGS = {
    "datalog": {
        "policy_type": "DATALOG",
        "policy_iterations": 10,
        "abort_on_cycle": True,
        "solve_prog": "solve_datalog.lp",
        "include_numerical_features": False,
        "include_concepts": True,
        "include_roles": True,
        "include_actions": False,
        "include_dead_states": False,
        "include_goal_states": False,
        "feature_generator": {
            "generate_concept_distance_numerical": False,
            "generate_count_numerical": False,
            "generate_til_c_role": True,
        },
    },
    "datalog-action-params": {
        "policy_type": "DATALOG",
        "policy_iterations": 1,
        "abort_on_cycle": True,
        "solve_prog": "solve_datalog_action_params.lp",
        "include_action_params": True,
        "include_roles": False,
        "include_concepts": False,
        "include_dead_states": False,
        "include_goal_states": False,
    },
    "datalog-actions": {
        "policy_type": "DATALOG",
        "policy_iterations": 1,
        "abort_on_cycle": True,
        "solve_prog": "solve_datalog_actions.lp",
        "include_pristine_states": False,
        "include_actions": True,
        "include_roles": False,
        "include_concepts": False,
        "include_action_params": False,
        "include_dead_states": False,
        "include_goal_states": False,
    },
    "exact": {
        "policy_type": "EXACT",
        "solve_prog": "solve.lp",
    },
    "state": {
        "policy_type": "CONSTRAINED",
        "solve_prog": "solve_fairness_assumption.lp",
    },
    "trans": {
        "policy_type": "CONSTRAINED",
        "solve_prog": "solve_trans_constraints.lp",
    },
    "d2l": {
        "policy_type": "EXACT",
        "policy_iterations": 1,
        "abort_on_cycle": True,
        "solve_prog": "solve_d2l.lp",
    },
}


class ConfigHandler(dict):

    def __init__(
        self, config_file_object: Optional[TextIO] = None, type: Optional[str] = None, override: Optional[dict] = None
    ):
        mergedeep.merge(self, DEFAULT_CONFIG)
        if type and type in DEFAULT_TYPE_CONFIGS:
            mergedeep.merge(self, DEFAULT_TYPE_CONFIGS[type])
        if config_file_object:
            mergedeep.merge(self, yaml.safe_load(config_file_object))
        if override:
            # Only override values that are already in the config that have been set to a non-None value
            mergedeep.merge(self, {k: v for k, v in override.items() if k in self and v is not None})

    def dump(self) -> str:
        dump: dict = dict()
        mergedeep.merge(dump, self)
        return yaml.dump(dump, default_flow_style=False)
